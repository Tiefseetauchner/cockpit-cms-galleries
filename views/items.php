        ...
        <template>
            ...
            <app-scrollcontainer mode="boundary" boundary="#items-actionbar">
                ...
                <div class="animated fadeIn kiss-height-50vh kiss-flex kiss-flex-middle kiss-flex-center kiss-align-center kiss-color-muted" v-if="fieldTypes && !loading && !items.length">
                    <div>
                        <kiss-svg class="kiss-margin-auto" src="<?php echo $this->base('content:assets/icons/collection.svg') ?>" width="40" height="40"><canvas width="40" height="40"></canvas></kiss-svg>
                        <p class="kiss-size-large kiss-text-bold kiss-margin-small-top"><?php echo t('No items') ?></p>
                    </div>
                </div>
                    ...
                    <div class="kiss-flex kiss-flex-middle">
                        <div class="kiss-flex kiss-flex-middle" v-if="!loading && count">
                            <app-pagination>
                                ...
                                <a class="kiss-margin-small-left" v-if="(page - 1) >= 1" @click="load(page - 1)"><?php echo t('Previous') ?></a>
                                ...
                                <a class="kiss-margin-small-left" v-if="(page + 1) <= pages" @click="load(page + 1)"><?php echo t('Next') ?></a>
                            </app-pagination>
                        </div>
                        ...
                        <div class="kiss-button-group">
                            <a class="kiss-button" href="<?php echo $this->route("/galleries") ?>"><?php echo t('Close') ?></a>
                            <?php if ($this->helper('acl')->isAllowed("content/{$model['name']}/create")) : ?>
                                <a class="kiss-button kiss-button-primary" @click="toggleBulkAddImage"><?php echo t('Add images') ?></a>
                            <?php endif ?>
                        </div>
                    </div>
                </kiss-container>
            ...
            <teleport to="body">
                ...
                <kiss-popout id="model-column-options" modal="true">
                    <kiss-content>
                        <kiss-navlist class="kiss-margin">
                            <ul>
                                <li class="kiss-nav-header"><?php echo t('Show Fields') ?></li>
                            </ul>
                        ...
                        <button type="button" class="kiss-button kiss-button-small kiss-width-1-1 kiss-margin-small-top" kiss-popout-close><?php echo t('Close') ?></button>
                    </kiss-content>
                </kiss-popout>
                ...
                <kiss-popout id="model-menu-actions">
                    <kiss-content>
                        <kiss-navlist class="kiss-margin">
                            <ul>
                                <li class="kiss-nav-header"><?php echo t('Collection actions') ?></li>
                                <li>
                                ...
                                <li>
                                    <a class="kiss-flex kiss-flex-middle" href="<?php echo $this->route("/content/collection/item/{$model['name']}") ?>">
                                ...
                                <li class="kiss-nav-divider"></li>
                                <li>
                                    <a class="kiss-flex kiss-flex-middle" href="<?php echo $this->route("/content/models/edit/{$model['name']}") ?>">
                                        <icon class="kiss-margin-small-right">create</icon>
                                        <?php echo t('Edit model') ?>
                                    </a>
                                </li>
                            </ul>
                        </kiss-navlist>
                ...
                <kiss-popout :open="actionBulkAddImage" @popoutclose="toggleBulkAddImage">
                    <kiss-content>
                        <div class="kiss-padding">
                            <h3 class="kiss-size-4 kiss-margin-bottom"><?php echo t('Upload Images') ?></h3>
                            <form @submit.prevent="uploadImages">
                                <div class="kiss-margin-bottom">
                                    <input type="file" ref="fileInput" multiple accept="image/*" @change="handleFileChange" class="kiss-width-1-1">
                                </div>
                                <div class="kiss-margin-top">
                                    <button type="submit" class="kiss-button kiss-button-primary kiss-width-1-1"><?php echo t('Upload') ?></button>
                                </div>
                            </form>
                            <app-loader class="kiss-margin" v-if="!fieldTypes || loading"></app-loader>
                        </div>
            ...
            <teleport to="#content-collection-items-aside">
                    ...
                    <ul>
                        <li>
                            <a class="kiss-link-muted kiss-flex kiss-flex-middle" :class="!selectedView ? 'kiss-text-bold':'kiss-color-muted'" gap="xsmall" @click="selectView(null)">
                                <icon :class="{'kiss-color-primary': !selectedView}">radio_button_checked</icon> <?php echo t('All items') ?>
                            </a>
                        </li>
                    </ul>
                </kiss-navlist>
                ...
                <kiss-navlist class="kiss-margin-small">
                    <ul>
                        <li class="kiss-nav-header"><?php echo t('My views') ?></li>
                        <li>
                            <a class="kiss-link-muted kiss-flex kiss-flex-middle" :class="selectedView == 'created-by-me' ? 'kiss-text-bold':'kiss-color-muted'" gap="xsmall" @click="selectView('created-by-me')">
                                ...
                                <span class="kiss-flex-1 kiss-text-truncate"><?php echo t('Created by me') ?></span>
                            </a>
                        </li>
                        <li>
                            <a class="kiss-link-muted kiss-flex kiss-flex-middle" :class="selectedView == 'updated-by-me' ? 'kiss-text-bold':'kiss-color-muted'" gap="xsmall" @click="selectView('updated-by-me')">
                                ...
                                <span class="kiss-flex-1 kiss-text-truncate"><?php echo t('Updated by me') ?></span>
                            </a>
                        </li>
                    </ul>
                </kiss-navlist>
        ...
        <script type="module">
            ...
            export default {
                ...
                data() {
                    ...
                    let model = <?php echo json_encode($model) ?>,
                    ...
                    return {
                        ...
                        acl: {
                            canCreate: <?php echo ($this->helper('acl')->isAllowed("content/{$model['name']}/create") ? 'true' : 'false') ?>,
                            canPublish: <?php echo ($this->helper('acl')->isAllowed("content/{$model['name']}/publish") ? 'true' : 'false') ?>
                        },
                ...
                methods: {
                    ...
                    uploadImages(event) {
                      ...
                      for (let file of files) {
                          ...
                          this.$request('/assets/upload', formData).then(asset => {
                              let newItem = {
                                item: {
                                  images: {
                              ...
                              this.$request(`/content/models/saveItem/${this.model.name}`, newItem).then(item => {
                                  App.ui.notify('Image uploaded and added to gallery!', 'success');
                                  this.load(); // Reload the list to show the new item
                              }).catch(error => {
                                  App.ui.notify('Failed to add item to gallery', 'error');
                              });
                          }).catch(error => {
                              App.ui.notify('Failed to upload image', 'error');
                          });
                      }
                    },
                    ...
                    toggleBulkAddImage() {
                        this.actionBulkAddImage = this.actionBulkAddImage === null ? true : null;
                    },
...
<div class="kiss-flex kiss-flex-middle kiss-margin" gap="small">
    <div>
        <kiss-svg class="kiss-margin-auto" src="<?php echo $this->base(isset($model['icon']) && $model['icon'] ? $model['icon'] : 'content:assets/icons/collection.svg') ?>" width="25" height="25" style="color:<?php echo ($this->escape($model['color'] ?? 'inherit')) ?>"><canvas width="25" height="25"></canvas></kiss-svg>
    </div>
    ...
    <div class="kiss-text-bold kiss-size-4 kiss-flex-1 kiss-text-truncate"><?php echo $this->escape($model['label'] ? $model['label'] : $model['name']) ?></div>
...
<div class="kiss-margin-small kiss-size-small kiss-color-muted">
    <?php echo $this->escape($model['info'])?>
</div>
<?php endif ?>
